...
export function connectChecks(win,expectedPersona=null){
 ...
 on('onADCSessionBound',e=>{const id=e.detail?.conversationId;if(typeof id!=='string'||retired.has(id)||(expectedPersona&&e.detail?.personaKey!==expectedPersona))return;if(id!==conversationId||ended){if(conversationId&&id!==conversationId)retired.add(conversationId);conversationId=id;card=null;ended=false;publish();}});
