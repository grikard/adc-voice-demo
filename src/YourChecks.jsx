...
export function YourChecks({controller,onReview,disabled,language='en'}){
 ...
 return <section className="checks-card" aria-labelledby="yourChecksTitle">
  ...
  </li>)}</ul><details className="checks-details"><summary>About these checks</summary><ul>{rows.map(row=><li key={row.key}><strong>{row.label}</strong><span>{row.origin} · {row.time}{row.timing&&` · ${row.timing}`}</span></li>)}</ul></details></>:<p className="checks-empty">{de?'Es liegen noch keine bestätigten Ergebnisse für dieses Gespräch vor.':'No verified results are available for this conversation yet.'}</p>}
  {card?.customerReports?.length>0&&<details className="checks-details" open><summary>Your reported updates · not sensor measurements</summary><ul>{card.customerReports.map(r=><li key={r.id}><strong>{r.text}</strong><span>Reported by you · {new Date(r.recordedAt).toLocaleString()}</span></li>)}</ul></details>}
